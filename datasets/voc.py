...
class VOCSegmentation_custom(data.Dataset):
    cmap = voc_cmap() #creates color map
    print("CMAP", len(cmap))
    ...
    def __init__(self,
        ...
        salmap_dir = os.path.join(self.root, opts.saliency_map)
        salmap_dir_old = os.path.join(self.root, 'saliency_map')
...
class VOCSegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        print(f"[INFORMATION]Saliency map being used from {opts.saliency_map}")
        ...
        self.masks = [os.path.join(mask_dir, x + ".png") for x in file_names]
        self.sal_maps = [os.path.join(salmap_dir, x + ".png") for x in file_names]
        self.sal_maps_old = [os.path.join(salmap_dir_old, x + ".png") for x in file_names]
        self.file_names = file_names
    ...
    def __getitem__(self, index):
        ...
        target = Image.open(self.masks[index])
        if self.image_set == 'train': #os.path.exists(self.sal_maps[index]):
            if os.path.exists(self.sal_maps[index]):
              sal_map = Image.open(self.sal_maps[index])
              #print("[INFORMATION] From inside")
            else:
              #print("[INFORMATION] Using from outside")
              sal_map = Image.open(self.sal_maps_old[index])
        else:
            # sal_map is useless for the valdation
            sal_map = Image.fromarray(np.ones(target.size[::-1], dtype=np.uint8))
        ...
        if self.image_set == 'train' and self.unknown:
            ...
            #print("The size of the target is ", target.size(), 'and the size of the sal_map is ', sal_map.size())
            
            
            #print("Sal map", sal_map[200][:15])
            unknown_area = (target == 1) & (sal_map > 0)
            target = torch.where(unknown_area, torch.zeros_like(target), target)
