...
def get_argparser():
    ...
    parser.add_argument("--saliency_map", default='saliency_map', type=str,
                        help="Path of the saliency map in the root model")
    parser.add_argument("--exp_str", type=str,
                        help="Additional input for name of the checkpoint string")
...
def validate(opts, model, loader, device, metrics):
    ...
    with torch.no_grad():
        for i, (images, labels, _, _) in enumerate(loader):
            ...
            #print("The output is ", outputs.size())
...
def main(opts):
    ...
    if opts.log_results:
      ...
      wandb.init(project="incre-seg",entity="incremental_seg", name=opts.log_name, allow_val_change=True)
      wandb.run.log_code(".")
    ...
    if opts.overlap:
        ckpt_str = "checkpoints/%s_%s_%s_%s_step_%d_overlap.pth"
    else:
        ckpt_str = "checkpoints/%s_%s_%s_%s_step_%d_disjoint.pth"
    
    if opts.curr_step > 0: # previous step checkpoint
        opts.ckpt = ckpt_str % (opts.model, opts.dataset, opts.task,opts.exp_str, opts.curr_step-1)
    ...
    model.train()
    save_ckpt(ckpt_str % (opts.model, opts.dataset, opts.task,opts.exp_str, opts.curr_step))
    ...
    while cur_itrs < total_itrs:
        ...
        if val_interval > 0 and (cur_itrs) % val_interval == 0:
            ...
            if opts.log_results:
              wandb.log({f'val_at_step_{opts.curr_step}':curr_score, f"epochs_for_{opts.curr_step}": cur_epochs})
            print()
            ...
            if curr_score > best_score:  # save best model
                ...
                if opts.log_results:
                  wandb.log({f'best_score_at_step_{opts.curr_step}':best_score, f"epochs_for_{opts.curr_step}": cur_epochs})
                save_ckpt(ckpt_str % (opts.model, opts.dataset, opts.task,opts.exp_str, opts.curr_step))
    ...
    if opts.curr_step > 0:
        ...
        best_ckpt = ckpt_str % (opts.model, opts.dataset,opts.task,opts.exp_str, opts.curr_step)
